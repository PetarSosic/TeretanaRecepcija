...
| D-69 | A password-reset link proves who the user is, as the temporary password does at first login: it opens **S-01b**, which asks only for the new password and blocks every other screen until it is saved (AS-18). The current password is not asked for. The Supabase "Reset Password" email template links to `{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=recovery`, so the link works once, in any browser and on any device, and does not depend on `APP_URL`; `/auth/callback` reads no destination from the URL. Amends US-01.4 AC1. Approved by the owner, 24.09.2026, after the reset link led back to the login screen (N-29). | O |
