-- M-13: visit statistics (F-20, S-15). Doc 07 §5 puts this one read function in the
-- hands of the owner and the manager, unlike the finance reports of doc 03 §14, which
-- stay the owner's alone (BR-157). Nothing here touches money.
 ...
 */
create function visit_stats(p_from date, p_to date)
returns json
language plpgsql
stable
security definer
...
begin
  ...
  return json_build_object(
    ...
    'days', coalesce((
      select json_agg(json_build_object('day', d.day, 'count', coalesce(c.count, 0))
                      order by d.day)
